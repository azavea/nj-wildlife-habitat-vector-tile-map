module "origin" {
  ...
  bucket_name          = "nj-wildlife-${lower(var.environment)}-site-${var.aws_region}"
  logs_bucket_name     = "nj-wildlife-${lower(var.environment)}-logs-${var.aws_region}"
...
resource "aws_cloudfront_distribution" "app" {
  ...
  enabled             = true
  http_version        = "http2"
  comment             = "NJ WILDLIFE (${var.environment})"
